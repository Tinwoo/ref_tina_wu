...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!(await knex.schema.hasTable("chatroom"))) {
    await knex.schema.createTable("chatroom", (table) => {
      table.increments();
      table.integer("users_left_id").unsigned().notNullable();
      table.foreign("users_left_id").references("users.id");
      table.integer("users_right_id").unsigned().notNullable();
      table.foreign("users_right_id").references("users.id");
      table.timestamps(false, true);
    });
  }
}
